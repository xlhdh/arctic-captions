...
def gen_sample_ensemble(tparams_list, f_init_list, f_next_list, ctx0, options,
    ...
    for ii in xrange(maxlen):
        ...
        next_p_list = []
        for fnid in range(len(ctx0_list)):
            #print next_w.shape, ctx0_list[0].shape, next_state_list[0].shape
            # our "next" state/memory in our previous step is now our "initial" state and memory
            rval = f_next_list[fnid](*([next_w, ctx0_list[fnid]]+next_state_list[fnid]+next_memory_list[fnid]))
            ...
            #next_state_list.append(next_state)
            #next_memory_list.append(next_memory)
            next_state_list[fnid]=next_state
            next_memory_list[fnid]=next_memory
        next_p = numpy.array(next_p_list).mean(axis=0)
        #print next_p.shape
        #next_w = numpy.random.choice(10000, size=3, p=next_p.reshape((10000,)))
